...
def upgrade() -> None:
    ...
    if _has_table("applications") and not _has_index("applications", "ix_applications_state"):
        op.create_index("ix_applications_state", "applications", ["state"])

    if not _has_table("application_stage_transitions"):
        op.create_table(
    ...
    if not _has_table("agent_events"):
        op.create_table(
            "agent_events",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
